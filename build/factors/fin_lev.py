...
★ 比率那批（倍 / 小数）横截面可比；「净债务」「净运营资本」「金融负债」
  那几条是**绝对额（元）**，2026-09-24 之前被一刀切判成
  `xs_comparable=False` —— **那是判错了**：元是全市场共同标度，可比。
  它们真正的毛病是**与市值高度重合**（实测 ttm+lev 共 28 个的 IC
  中位只有 −0.0079、|IC| > 0.03 的一个都没有）—— 那是「有没有用」，
  不是「可不可比」，两件事别混。
"""

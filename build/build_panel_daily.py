...
# 🔴 复权后收益超过当日涨跌停限制的【已知坏行】白名单（2026-09-04 定案，16 行）。
#   涨跌停限制是硬约束，而 close_hfq 的 ret_1d 是【复权后】收益 ——
#   所以它超过 limit_pct 只有三种可能：上市初期无限制、停牌复牌跳空、
#   **复权因子算错**。前两种在查询里排除掉，剩下的就该是空的。
#   用白名单而不是放宽阈值 —— 任何【新】案例都会被断言抓到
#   （同 KNOWN_PRELIST_KLINE / KNOWN_VWAP_BELOW_LOW 那条原则）。
#
#   查全库 9,594 个「聚宽有送转 + tdx 同日有 K 线」的除权事件（终极判据：
#   close 跌幅≈理论除权倍数 说明交易所确实除权，此时 factor 必须同步跳）：
#     ✓ 9,450 (98.5%)  除权与复权都对
#     ✓    24          close 与 factor 都没动 —— 交易所确实没除权，tdx 是对的
#                      （破产重整的资本公积转增：股份给债权人、不按股东比例，
#                        聚宽的 transfer_ratio 记了它但交易所不除权。
#                        🔴 所以**聚宽的 transfer_ratio 不能直接当除权比例用**）
#     🔴    5          漏复权 —— **全部是 300114.XSHE**
#     （另有 31 个"价没跌 factor 却跳了"是假警：理论倍数都是 ×1.1~1.2 的小除权，
#       除权当天涨停正好抵消 —— 22.00/1.2×1.2 = 22.0，factor 是对的）
#
# 🔴 **300114.XSHE 中航电测：tdx 的 raw_gbbq 里 0 条记录，hfq_factor 恒为
#   1.000000，5 次送转全漏**（2011送5 / 2013送3 / 2015转3 / 2016转5 / 2017转5，
#   聚宽累积 ×5.704）。所以它的 close_hfq **就等于 close_bfq**，跨那 5 个除权日
#   会算出 −33.5% / −22.1% / −17.3% / −34.2% / −33.6% 的**假跌幅**。
#   ★ 根因与本文件另一处注释同源：该股 tdx 股本数据缺失（floatmv=totalmv=0）。
#   ★ 影响方向是**虚减**收益（保守），且它在面板里 2025-02-14 之后就没有数据了，
#     所以只影响历史回测。**没修**：修要在 mart 层为一只票引入复权重算
#     （新职责），且得重建 2011 年起全部年份。先看住不让它扩大。
#
# 🔴 **002204.XSHE 2014-07-28：raw_gbbq 的脏数据。** 那条 cat=1 记录 c3=8.22
#   （c3 = 每 10 股送转股数，实测 c3=5→factor×1.5100、c3=10→×2.0037 可验），
#   于是 factor 跳 ×1.8218。但三项独立证据都说它没送转：
#     · 总股本 2014-07-25 → 07-28 完全没变（9.6569 亿），送转必然改股本
#     · 真送转都有配套的 cat=9 记录（该股 2012/2013/2016 三次都有），这天没有
#     · 聚宽 dividend 里这一天没有任何除权记录
#   后果：那天 ret_1d 报 +64.0%，而实际是 **−10% 一字跌停**（O=H=L=C=17.01）。
#   之后的 close_hfq 整段偏高 82%（比例关系仍对，只有跨那一天时才错）。
KNOWN_RET_OVER_LIMIT = {
    ('000536.XSHE', '2014-09-25'),
    ('000725.XSHE', '2009-06-10'),
    ('000738.XSHE', '2010-10-15'),
    ('000760.XSHE', '2009-06-12'),
    ('000982.XSHE', '2024-06-13'),
    ('000982.XSHE', '2024-06-18'),
    ('002108.XSHE', '2009-03-05'),
    ('002204.XSHE', '2014-07-28'),        # gbbq 脏数据，见上
    ('300090.XSHE', '2020-05-28'),
    ('300114.XSHE', '2011-05-13'),        # 以下 5 行：漏复权，见上
    ('300114.XSHE', '2013-06-21'),
    ('300114.XSHE', '2015-05-27'),
    ('300114.XSHE', '2016-05-25'),
    ('300114.XSHE', '2017-05-26'),
    ('600481.XSHG', '2008-01-21'),
    ('600660.XSHG', '2009-04-21'),
}
...
def verify(con):
    ...
    # 5a4) 🔴 复权因子：**复权后**收益不该超过当日涨跌停限制
    #   涨跌停是硬约束，close_hfq 的 ret_1d 超过它只剩三种可能 ——
    #   上市初期无限制 / 停牌复牌跳空 / **复权因子算错**。前两种排除掉。
    #   ★ 只查 public_status='正常上市'：ST 与退市整理期的涨跌幅规则本身特殊
    #     （破产重整的资本公积转增，交易所口径与 tdx/聚宽三方都不一致），
    #     那批 321 行性质明确、不适合逐行白名单。
    #   ★ rn > 5 用的是【交易日序】而不是 listed_days ——
    #     listed_days 是自然日，2024-10-09 那批次新股 listed=13 但只是第 5 个
    #     交易日、仍在无限制期，按自然日判会误报 3 只（实测踩过）。
    bad = [(a, d) for a, d in con.execute("""
        WITH b AS (
            SELECT jq_code, date, ret_1d, limit_pct, public_status,
                   lag(date) OVER (PARTITION BY jq_code ORDER BY date) pd,
                   row_number() OVER (PARTITION BY jq_code ORDER BY date) rn
            FROM %s)
        SELECT jq_code, CAST(date AS VARCHAR) FROM b
        WHERE ret_1d IS NOT NULL AND limit_pct > 0
          AND abs(ret_1d) > limit_pct * 1.1
          AND rn > 5 AND date_diff('day', pd, date) <= 5
          AND date >= DATE '2007-01-01'
          AND public_status = '正常上市'""" % P).fetchall()
           if (a, d) not in KNOWN_RET_OVER_LIMIT]
    check(not bad, '复权后收益 <= 涨跌停限制（复权因子正确）—— 白名单外 %d 行 %s'
          % (len(bad), (bad[:5] if bad else '')))
